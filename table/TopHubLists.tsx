...
const TopHubLists = () => {
  ...
  const getMonthOptions = () => {
    ...
    for (let i = 0; i < 12; i++) {
      ...
      const displayName = date.toLocaleString("default", { month: "long", year: "numeric" })
              ...
              <table className="w-full">
                <thead className="bg-[#b0e0cf] font-semibold">
                  <tr className="text-center">
                    <th className="p-3 text-xs font-medium">Top Receiver Hub</th>
                ...
                <tbody>
                  {tableData?.data?.map((row : any, i : any) => (
                    <tr key={i} className="text-center bg-[#d9f0e8] border-b border-gray-300">
                      <td className="p-3 text-xs">{row.topReceiverHub || "N/A"}</td>
                      <td className="p-3 text-xs">{row.totalReceived || 0}</td>
                      <td className="p-3 text-xs">{row.topSenderHub || "N/A"}</td>
