...
export default function AdminPanel() {
  ...
  const { data: receivers = [] } = useQuery({
    ...
    queryFn: async () => {
      ...
      const res = await fetch(
        `${process.env.NEXT_PUBLIC_API_URL}/hub-manager/receiver-requests?search=${searchTerm}`,
        {
          headers: {
  ...
  return (
    <div className="mt-20">
      ...
      <div>
        <div className="mb-6 mt-6 bg-[#e6f5f0] rounded-lg p-4">
              ...
              <table className="min-w-full bg-[#e6f5f0]">
                ...
                <tbody>
                  {currentItems.map((item: any, i: any) => (
                    <tr
                      key={i}
                      className="border-b border-gray-200 bg-[#e6f5f0] text-sm"
                    >
                      <td className="py-4">
